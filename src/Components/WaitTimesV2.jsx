...
function WaitTimesV2() {
  ...
  const sortedData = useMemo(() => {
    return [...attractionsData].sort((a, b) => {
      // Define a status order: Operating first, then Boarding Group, then Closed, then Refurbishment
      const statusOrder = { "OPERATING": 0, "BOARDING_GROUP": 1, "CLOSED": 2, "REFURBISHMENT": 3 };
      
      // Get the status for each attraction
      ...
      // If both are operating or both are boarding group, sort by wait time
      if (statusA === "OPERATING" || statusA === "BOARDING_GROUP") {
        ...
        if (statusA === "OPERATING") {
          ...
          // For boarding groups, you might want to sort by some other criteria
          // For now, we'll just maintain their original order
          return 0;
        }
      ...
      // If both are closed or refurbishment, maintain their original order
      return 0;
    });
  ...
  {ride.status === "OPERATING" && (ride.operatingHours?.length || 0) > 0 ? (
    ride.queue?.BOARDING_GROUP ? (
      ...
      // Regular standby wait time
      <React.Fragment>
        <span className={styles.bold}>{ride.queue.STANDBY.waitTime}</span>{" "}
        mins
      </React.Fragment>
    ) : (
      // No wait time available
      'N/A'
    )
  ) : (
    // Ride is not operating
